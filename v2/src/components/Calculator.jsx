...
function Calculator() {
  ...
  const handleLogin = (e) => {
    ...
    if (password === PASSWORD) {
      setIsAuthenticated(true);
      sessionStorage.setItem('komfi-auth', 'true');
      setError('');
    } else {
        ...
        <div className="w-full max-w-md">
          <div className={`bg-white rounded-2xl p-8 md:p-10 shadow-sm border border-[#e8e5de] ${mounted ? 'animate-scale-in' : 'opacity-0'}`}>
            ...
            <form onSubmit={handleLogin}>
              ...
              <button
                type="submit"
                className="w-full btn-primary py-3 px-6 rounded-xl font-medium text-white shadow-sm hover:shadow-md transition-all"
              >
                <span>Vstoupit</span>
              </button>
            </form>
